...
#   scripts/release.sh --skip-test  # skip TestPyPI entirely, confirm, then publish to PyPI
#
...
# scoped to the `instantlyai` project once it exists there). Export them in your
# shell -- never commit them or put them in a file tracked by git:
#   export TESTPYPI_TOKEN=pypi-...   # not needed with --skip-test
...
case "${1:-}" in
  ...
  "") ;;
  *)
    echo "Unknown option: $1 (expected --test-only or --skip-test)" >&2
    exit 1
    ;;
esac
...
EOF
  ...
  echo "==> Skipping TestPyPI (--skip-test)"
fi

read -r -p "Publish ${VERSION} to real PyPI? [y/N] " confirm
